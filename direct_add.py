...
def itr_grads(cgd_data, x, ckpt_final, itr, idx):
    ...
    if do_each_grad == 1:
        ...
        for each in cgd_data:
            ...
            each_4d = np.expand_dims(each, axis=0)
            each_pred_lb_a = np.argmax(deep_cnn.softmax_preds(each_4d, ckpt_final))
        ...
        for batch_nb in range(batch_nbs):
            ...
            if batch_nb == 0 and itr == 0:
                for i in range(10):
                    img_dir = FLAGS.image_dir + '/' + str(FLAGS.dataset) + '/changed_data/x_grads/number_' + str(
                        idx) + '/' + 'img_' + str(i) + '/iteration_' + str(itr) + '_ori.png'
